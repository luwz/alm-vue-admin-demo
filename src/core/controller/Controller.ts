...
export abstract class Controller {
	...
	public static async async(callback: <T>(T?) => any) {
		...
		try {
			...
			if (typeof response.data.status !== "undefined" && response.data.status) {
				return typeof response.data.data !== "undefined" && response.data.data ? response.data.data : [];
			} else {
				toast.error(response.data.message);
				if (typeof response.data.errorCode !== "undefined" && response.data.errorCode === "401") {
		...
		callback().then((response: Result) => {
			if (typeof response.data.status !== "undefined" && response.data.status) {
				if (successCallback != null) {
					const rData = typeof response.data.data !== "undefined" && response.data.data ? response.data.data : [];
	 ...
	 * @param callback
	 */
	public static async asyncMock(callback: <T>(T?) => any) {
		const router: Router = useRouter();
		try {
			const response: Result = await callback();
	 ...
	 * @param errorCallback 失败回调
	 */
	public static syncMock(callback, successCallback?: <T>(T?) => any, errorCallback?: <T>(T?) => any) {
		const {push}: Router = useRouter();
